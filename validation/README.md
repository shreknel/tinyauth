...
### nginx
- **Purpose:** Reverse proxy for `auth.aande.top` → tinyauth
- **Ports:** 80 (exposed to host)
- **Access:** http://auth.aande.top/ (via nginx on port 80)

### dns
- **Purpose:** DNS server (dnsmasq) that resolves `auth.aande.top` to the tinyauth container
- **Configuration:** Resolves `auth.aande.top` to the `tinyauth` container IP (172.28.0.20) within the Docker network
- **Ports:** 53 (UDP/TCP) - not exposed to host (only for container-to-container communication)

### tinyauth
- **URL:** http://auth.aande.top/ (via nginx)
- **Credentials:** `user` / `pass`
- **OIDC Discovery:** http://auth.aande.top/api/.well-known/openid-configuration
...
2. **Launch Chrome with host-resolver-rules:**
   ...
   google-chrome \
     ...
     --unsafely-treat-insecure-origin-as-secure=http://auth.aande.top \
...
3. **Access tinyauth:** http://auth.aande.top/
...
Since the DNS server is only accessible within the Docker network, you have several options to access `auth.aande.top` from your browser:
...
```
127.0.0.1 auth.aande.top
```

Then access: http://auth.aande.top/

**To edit /etc/hosts on Linux/Mac:**
```bash
sudo nano /etc/hosts
# Add: 127.0.0.1 auth.aande.top
```

**To edit hosts on Windows:**
1. Open Notepad as Administrator
2. Open `C:\Windows\System32\drivers\etc\hosts`
3. Add: `127.0.0.1 auth.aande.top`
...
2. **Launch Chrome with the host resolver rule:**
   ...
   **Linux:**
   ```bash
   google-chrome --host-resolver-rules="MAP auth.aande.top 127.0.0.1"
   ```
   ...
   **Windows:**
   ```cmd
   "C:\Program Files\Google\ Chrome\Application\chrome.exe" --host-resolver-rules="MAP auth.aande.top 127.0.0.1"
   ```

3. **Or modify Chrome's shortcut:**
   - Right-click Chrome shortcut → Properties
   - In "Target" field, append: ` --host-resolver-rules="MAP auth.aande.top 127.0.0.1"`
   - Click OK

4. **Access:** http://auth.aande.top/
...
3. Navigate to: http://auth.aande.top/
4. Login with `user` / `pass`
5. Test the OIDC flow by accessing the discovery endpoint: http://auth.aande.top/api/.well-known/openid-configuration
...
- App URL and OIDC issuer: `http://auth.aande.top` (via nginx on port 80)

## Notes

- All containers are on a custom Docker network (`tinyauth-network`) with a DNS server for domain resolution
- The DNS server resolves `auth.aande.top` to the tinyauth container within the network
- The redirect URI must match exactly what's configured in tinyauth
- Data is persisted in the `./data` directory
- The domain `auth.aande.top` is used to satisfy cookie domain validation requirements (needs at least 3 domain parts and not in public suffix list)
